...
router.post("/submit", isAuthenticatedUser, async (req, res) => {
  try {
    ...
    const userRole = req.user.role === "teacher" ? "Teacher" : req.user.role === "admin" ? "Admin" : "Student";
    ...
    res.status(201).json({ message: "Result submitted successfully", resultId: newResult._id });
  } catch (error) {
